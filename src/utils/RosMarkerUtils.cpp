...
void RosMarkerUtils::init_quad_plus( visualization_msgs::Marker& marker, float L )
{
    ...
    // drone landing gear
    #if 1
    geometry_msgs::Point pu, pd;
    pu.x = L; pu.y = 0.0 ; pu.z = 0.0;
    pd.x = L; pd.y = 0.0 ; pd.z = -0.3*L;
    marker.points.push_back( pu );
    marker.points.push_back( pd );
    marker.colors.push_back( c_white );
    marker.colors.push_back( c_white );

    pu.x = 0.0; pu.y = L ; pu.z = 0.0;
    pd.x = 0.0; pd.y = L ; pd.z = -0.3*L;
    marker.points.push_back( pu );
    marker.points.push_back( pd );
    marker.colors.push_back( c_white );
    marker.colors.push_back( c_white );
